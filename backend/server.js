...
app.post("/api/signup", async (req, res) => {
  ...
  if (!nom || !email || !pass) {
    return res.status(400).json({ message: "Champs requis manquants" }); // Translated message
  }
  ...
  try {
    ...
    if (existingUser.rows.length > 0) {
      return res.status(400).json({ message: "L'email existe déjà" }); // Translated message
    }
    ...
    res.status(500).json({ message: "Erreur serveur, veuillez réessayer plus tard" }); // Translated message
  }
});
...
app.post("/api/signin", async (req, res) => {
  ...
  if (!email || !pass) {
    return res.status(400).json({ message: "L'email et le mot de passe sont requis" }); // Translated message
  }
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({ message: "Email ou mot de passe invalide" }); // Translated message
    }
...
app.post("/api/forgot-password", async (req, res) => {
  ...
  if (!email) {
    return res.status(400).json({ message: "L'email est requis" }); // Translated message
  }
  ...
  try {
    ...
    if (!user) {
      return res.status(200).json({ message: "Si l'email existe, un lien de réinitialisation a été envoyé." }); // Translated message
    }
    ...
    const mailOptions = {
      ...
      to: email,
      subject: "Demande de réinitialisation de mot de passe", // Translated message
      text: `Cliquez sur ce lien pour réinitialiser votre mot de passe : ${resetUrl}\nCe lien expire dans 1 heure.`, // Translated message
    };

    await transporter.sendMail(mailOptions);

    res.status(200).json({ message: "Si l'email existe, un lien de réinitialisation a été envoyé." }); // Translated message
  } catch (error) {
...
app.post("/api/reset-password", async (req, res) => {
  ...
  if (!token || !newPassword) {
    return res.status(400).json({ message: "Le jeton et le nouveau mot de passe sont requis" }); // Translated message
  }
  ...
  try {
    ...
    if (!resetToken || resetToken.user_id !== decoded.id) {
      return res.status(400).json({ message: "Jeton de réinitialisation invalide ou expiré" }); // Translated message
    }
    ...
    res.status(200).json({ message: "Mot de passe réinitialisé avec succès ! Vous pouvez maintenant vous connecter." }); // Translated message
  } catch (error) {
    console.error("Error in reset-password:", error);
    if (error.name === "TokenExpiredError") {
      return res.status(400).json({ message: "Le jeton de réinitialisation a expiré" }); // Translated message
    }
    res.status(500).json({ message: "Erreur serveur, veuillez réessayer plus tard" }); // Translated message
  }
});
...
app.post("/api/contact", async (req, res) => {
  ...
  if (!fullName || !email || !comments) {
    return res.status(400).json({ message: "Tous les champs sont requis" }); // Translated message
  }
  ...
  const mailOptions = {
    ...
    subject: `Nouvelle soumission de formulaire de contact de ${fullName}`, // Translated message
    text: `
  ...
  try {
    await transporter.sendMail(mailOptions);
    res.status(200).json({ message: "Message envoyé avec succès" }); // Translated message
  } catch (error) {
    console.error("Error sending contact email:", error);
    res.status(500).json({ message: "Échec de l'envoi du message" }); // Translated message
  }
});
